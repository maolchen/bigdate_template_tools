...
本项目现在包含一个可视化的 Web 配置编辑器，通过浏览器即可管理 `config.yaml` 配置：
...
| 功能 | 说明 |
|------|------|
| **节点管理** | 添加、编辑、删除节点，自动检查节点引用 |
| **全局配置** | 编辑全局变量（用户、目录、Java 路径等） |
| **服务拓扑** | 配置服务部署位置，支持多节点选择和全局服务 |
| **服务配置** | 管理服务详细参数，支持 JSON 格式变量 |
| **YAML 预览** | 实时预览生成的 config.yaml，带验证提示 |
