...
function templateKind(entry: ConfigTemplateEntry): 'main' | 'backup' {
  if (entry.kind === 'main' || entry.id === 'config') return 'main';
  return 'backup';
}
...
export function ConfigManagementPage({
  ...
  const loadTemplates = async () => {
    ...
    try {
      ...
      setTemplateError(error instanceof Error ? error.message : '获取模板列表失败');
    } finally {
  ...
  const handleCreateTemplate = async () => {
    ...
    try {
      ...
      setSyncMessage('已从主模板 config.yaml 创建备份模板');
    } catch (error) {
      setTemplateError(error instanceof Error ? error.message : '保存备份模板失败');
    } finally {
    ...
    try {
      ...
      setSyncMessage(
        response.sync.serviceTopAdded > 0 || response.sync.serverConfigAdded > 0
          ? `已切换为主模板（互换），并补齐 ${response.sync.addedServices.length} 个新增服务`
          : '已切换为主模板（互换）',
      );
      await loadTemplates();
    } catch (error) {
      setTemplateError(error instanceof Error ? error.message : '切换主模板失败');
    } finally {
  ...
  const handleDeleteTemplate = async (templateId: string) => {
    if (!confirm(`确认删除备份模板 ${templateId} 吗？`)) return;
    setDeletingTemplateId(templateId);
    setTemplateError(null);
    try {
      setTemplates(await deleteConfigTemplate(templateId));
    } catch (error) {
      setTemplateError(error instanceof Error ? error.message : '删除备份模板失败');
    } finally {
      ...
      {!hidePageTitle && (
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-500 mt-1">模板管理、节点管理、服务拓扑与服务配置统一维护</p>
          </div>
        </div>
      )}
        ...
        <div className="space-y-6">
          <div className="card">
            <div className="card-body">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="form-label">备份模板 ID</label>
                  <input
                    className="input"
                    value={templateIdInput}
                    onChange={(event) => setTemplateIdInput(event.target.value)}
                    placeholder="例如: project_4node"
                  />
                </div>
                <div>
                      ...
                      <tr key={item.id}>
                        <td>
                          <div className="flex items-center gap-2">
                            ...
                            {isMain && <span className="badge badge-green text-xs">当前主模板</span>}
                          </div>
                        </td>
                        <td>
                          <span className={`badge text-xs ${isMain ? 'badge-green' : 'badge-gray'}`}>
                            {isMain ? '主模板(config.yaml)' : '备份模板'}
                          </span>
                        </td>
                        <td>{item.fileName}</td>
                        <td>{item.remark || '-'}</td>
                        ...
                        <td>
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              disabled={switchingTemplateId === item.id || isMain}
                              title={isMain ? '主模板无需切换' : '切换为主（互换）'}
                            >
                              切换为主（互换）
                            </button>
                            <button
                              ...
                              disabled={deletingTemplateId === item.id || isMain}
                              title={isMain ? '主模板不可删除' : '删除备份模板'}
                            >
          ...
          <div className="card">
            <div className="card-body text-sm text-gray-600">
              <p>
                当前主模板: <span className="font-medium text-gray-800">{activeTemplate?.id ?? activeTemplateId}</span>
              </p>
              <p className="mt-1">
                主模板固定为 config.yaml。切换备份模板时采用主备互换，当前主模板会自动保留为备份，避免配置丢失。
              </p>
            </div>
          </div>
        </div>
