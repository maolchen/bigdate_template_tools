...
RM_PORT=8032
RM_READY=false
echo "  检查 ResourceManager: dw-master1:192.168.10.10:${RM_PORT}"
if timeout 5 bash -c "echo > /dev/tcp/192.168.10.10/${RM_PORT}" 2>/dev/null; then
    echo "    [✓] 连接正常"
    RM_READY=true
fi
echo "  检查 ResourceManager: dw-master2:192.168.10.11:${RM_PORT}"
if timeout 5 bash -c "echo > /dev/tcp/192.168.10.11/${RM_PORT}" 2>/dev/null; then
    echo "    [✓] 连接正常"
    RM_READY=true
fi
