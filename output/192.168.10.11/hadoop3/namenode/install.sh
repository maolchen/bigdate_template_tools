#!/bin/bash
# Hadoop 3 统一安装脚本
# 使用方法: bash install.sh [--force]
#   --force  强制重新安装（会备份已有目录）
...
CONFIG_DIR="${HADOOP_INSTALL_DIR}/etc/hadoop"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            log_error "未知参数: $1"
            echo "使用方法: bash install.sh [--force]"
            exit 1
            ;;
    esac
done
...
if [ -d "${HADOOP_INSTALL_DIR}/etc" ]; then
    if [ "$FORCE_INSTALL" = "true" ]; then
        ...
        log_error "Hadoop已安装在: ${HADOOP_INSTALL_DIR}"
        log_error "如需重新安装，请使用 --force 参数"
        exit 1
    fi
fi
...
if [ "$RUN_USER" = "root" ]; then
    ...
    if ! grep -q "HADOOP_HOME" /etc/profile 2>/dev/null; then
        echo "" >> /etc/profile
    ...
    if ! grep -q "HADOOP_HOME" ~/.bash_profile 2>/dev/null; then
        echo "" >> ~/.bash_profile
    ...
    # 同时添加到 /etc/profile（需要sudo权限）
    if ! run_as_root grep -q "HADOOP_HOME" /etc/profile 2>/dev/null; then
