...
for cfg_file in "${CONFIG_FILES[@]}"; do
    if [ -f "${SCRIPT_DIR}/${cfg_file}" ]; then
        ...
        run_as_root chmod 644 "${CONFIG_DIR}/${cfg_file}"
        log_info "已部署: ${cfg_file}"
    else
        log_warn "配置文件不存在，跳过: ${cfg_file}"
    fi
done
...
run_as_root rm -f "${WORKERS_FILE}"

run_as_root chown ${RUN_USER}:${RUN_GROUP} "${WORKERS_FILE}"
run_as_root chmod 644 "${WORKERS_FILE}"
