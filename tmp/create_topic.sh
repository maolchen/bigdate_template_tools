...
#1.etl-gate
bin/kafka-topics.sh --zookeeper localhost:2182 --create --topic sdklua_online --partitions=5 --replication-factor 2;
bin/kafka-run-class.sh kafka.tools.UpdateOffsetsInZK earliest gate-consumer.properties sdklua_online;

#2.etl-debug
bin/kafka-topics.sh --zookeeper localhost:2182 --create --topic debug --partitions=5 --replication-factor 2;
bin/kafka-run-class.sh kafka.tools.UpdateOffsetsInZK earliest debug-consumer.properties debug;

#3.etl-id
bin/kafka-topics.sh --zookeeper localhost:2182 --create --topic pay_statisv2 --partitions=5 --replication-factor 2;
...
bin/kafka-topics.sh --zookeeper localhost:2182 --create --topic pay_zg_total --partitions=5 --replication-factor 2;
...
#8.es es_index
#bin/kafka-topics.sh --zookeeper localhost:2182 --create --topic es_index --partitions=5 --replication-factor 2

# v4.3.0新增 标签评价管理
bin/kafka-topics.sh --zookeeper localhost:2182  --create  --topic tag-evaluate-new   --partitions 1   --replication-factor 1;
bin/kafka-topics.sh --zookeeper localhost:2182  --create  --topic file-task   --partitions 1   --replication-factor 1;

#v4.3.6新增 群组用户导入
bin/kafka-topics.sh --zookeeper localhost:2182  --create  --topic group-user-load  --partitions 1   --replication-factor 1;
