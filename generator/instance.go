...
func BuildServiceInstances(cfg *bigdata.Config) []bigdata.ServiceInstance {
	var instances []bigdata.ServiceInstance
	...
	for serviceName, topo := range cfg.ServiceTop {
		...
		if !exists {
			// 如果没有配置，使用空配置
			serviceConfig = bigdata.ServiceConfig{
		...
		for idx, nodeName := range targetNodes {
			...
			instances = append(instances, bigdata.ServiceInstance{
				ServiceName: serviceName,
				NodeName:    nodeName,
				Node: bigdata.NodeInstance{
					IP:       node.IP,
