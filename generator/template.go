...
import (
	...
	"bigdata-deploy-generator/utils"
)

// RenderTemplate 渲染模板
func RenderTemplate(tmplPath string, ctx bigdata.Context, cfg *bigdata.Config) (string, error) {
	// 读取模板文件
	...
	tmpl, err := template.New("config").Funcs(template.FuncMap{
		...
		// =============== 服务节点相关函数 ===============
		// 获取同一服务的所有节点实例
		"serviceNodes": func(serviceName string) []bigdata.ServiceInstance {
			var result []bigdata.ServiceInstance
		...
		// =============== 配置访问函数 ===============
		// 获取服务配置
		"serviceConfig": func(serviceName string) bigdata.ServiceConfig {
			return cfg.ServerConfig[serviceName]
		},

		// 获取节点信息
		"nodeInfo": func(nodeName string) *bigdata.Node {
